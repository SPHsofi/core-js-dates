...
function getNextFriday(date) {
  ...
  if (!daysToFriday) {
    nextFriday.setDate(inputDate.getDate() + 7);
  } else {
    nextFriday.setDate(inputDate.getDate() + daysToFriday);
  }
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (lastDate - date) / (1000 * 60 * 60 * 24) + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= lastDay.getDate(); i += 1) {
    newDate.setDate(i);
    if (newDate.getDay(i) === 6 || newDate.getDay(i) === 0) {
      weekend += 1;
    }
  }
  return weekend;
...
function getWeekNumberByDate(date) {
  ...
  const pastDays = Math.floor((date - firstDayOfYear) / millisecondsPerDay);
  const firstDayOfYearWeekday = firstDayOfYear.getDay() || 7;
  const weekNumber = Math.ceil((pastDays + firstDayOfYearWeekday) / 7);
